...
class PDFROW extends FPDF
{
    ...
    public $TitleSize='12';
    public $TitleStyle='B';
    public $FooterFont='Arial';
    public $FooterSize='8';
    public $FooterStyle='I';
    public $TextFont='Arial';
    public $TextSize='8';
    public $TextStyle='';
    public $Logo;
    public $cMarginTmp;
    ...
    public function SetTitle($title, $isUTF8=true)
    {
        ...
        parent::SetTitle($title, false);
        $this->Title=$title;
    }

    public function SetTitleFont($font, $style, $size)
    {
        $this->TitleSize=$size;
        $this->TitleFont=$font;
    ...
    public function SetFooterFont($font, $style, $size)
    {
    ...
    public function SetTextFont($font, $style, $size)
    {
        $this->TextSize=$size;
        $this->TextFont=$font;
    ...
    public function SetFooterText($footer, $isUTF8=true)
    {
        if ($isUTF8) {
    ...
    public function Header()
    {
        ...
        if (isset($this->Title)) {
            ...
            $this->Cell(30);
            //Title
            $this->Cell(15, 6, $this->Title, 0, 1, 'L');
            //Line break
            $this->Ln(1);
        }
    ...
    public function TableHeader()
    {
        ...
        foreach ($this->aCols as $col) {
            $value=$col['c'];
            $nb=max($nb, $this->NbLines($col['w'], $value));
        }
        $h=6*$nb;
        $w=0;
        foreach ($this->aCols as $col) {
            ...
            } elseif (!empty($col['colColor'])) {
                $fill=true;
                $this->SetFillColor($col['colColor'][0], $col['colColor'][1], $col['colColor'][2]);
            }

            $x=$this->GetX();
            $y=$this->GetY();
            $nb=$this->NbLines($col['w'], $col['c']);
            $this->MultiCell($col['w'], $h/$nb, $col['c'], $col['b'], $col['a'], $fill);
    ...
    public function RowBorder($h=2)
    {
        //Draw the borders
        $x=$this->TableX;
        $y=$this->GetY();
        $this->Line($x, $y, $x+$this->TableWidth, $y);
        $this->Ln($h);
        $this->SetX($this->TableX);
    }
    public function Row($data, $vpadding=4, $fontStyle='', $RowColors=array(), $fontsize=0)
    {
        $this->SetX($this->TableX);
        if ($fontsize!=0) {
            $this->TextSize=$fontsize;
        }
        $this->SetFont($this->TextFont, $fontStyle, $this->TextSize);
        ...
        foreach ($this->aCols as $col) {
            ...
            $y=$this->GetY();
            $fill=false;
            if (!empty($col['colColor'])) {
                //colColor
                $fill=true;
                $this->SetFillColor($col['colColor'][0], $col['colColor'][1], $col['colColor'][2]);
            } elseif (!empty($RowColors)) {
                $fill=true;
                $this->SetFillColor($RowColors[0], $RowColors[1], $RowColors[2]);
            }
            //Print the text
            $value=isset($data[$col['f']]) ? $data[$col['f']] : '';
            /*while ($this->NbLines($w, $value)<$nb) {
                $value .= "\n";
            }*/
            if ($value==="[]") { //checkbox
                $xc=$this->x;
                ...
                if ($fill) {
                    $this->Cell($w, $vpadding, "", 0, 0, $col['a'], true);
                }
                $this->Rect($xc, $this->y, 3, 3);
            } else {
                $this->MultiCell($w, $vpadding, $value, $col['b'], $col['a'], $fill);
            }
    ...
    *Compute the widths of the columns
    ...
    public function CalcWidths($width, $align)
    {
        ...
        if ($align=='C') {
            $this->TableX=max(($this->w-$this->TableWidth)/2, 0);
        } elseif ($align=='R') {
            $this->TableX=max($this->w-$this->rMargin-$this->TableWidth, 0);
        } else {
    ...
    *Add a column to the table
     */
    public function AddCol($field=-1, $width=-1, $caption='', $align='L', $border=0, $colColor=array())
    {
        if ($field==-1) {
            $field=count($this->aCols);
        }
        $this->aCols[]=array('f'=>$field, 'c'=>$caption, 'w'=>$width, 'a'=>$align, 'b'=>$border, 'colColor'=>$colColor);
    }
    ...
    *Page footer
    ...
    public function HandleProps($prop)
    {
        if (!isset($prop['width'])) {
            $prop['width']=0;
        }
    ...
    public function BeginTable($prop=array())
    {
        $prop=$this->HandleProps($prop);
        $this->cMarginTmp=$this->cMargin;
    ...
    public function EndTable($prop=array())
    {
        //Draw the borders
        $x=$this->TableX;
        $y=$this->GetY();
        ...
        $this->cMargin=$this->cMarginTmp;
        $this->ProcessingTable=false;
        $this->aCols=array();
    }
    public function Texte($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='')
    {
        $this->SetFont("", "");
        $this->Cell($w, $h, iconv('UTF-8', 'windows-1252', $txt), $border, $ln, $align, $fill, $link);
    }
    public function Valeur($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='')
    {
        $this->SetFont("", "B");
        $this->Cell($w, $h, iconv('UTF-8', 'windows-1252', $txt), $border, $ln, $align, $fill, $link);
    }
